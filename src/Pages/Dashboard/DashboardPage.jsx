...
const FileEditor = ({ file, onSave, onClose, user }) => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const fileRef = doc(db, 'users', user.uid, 'items', file.id);
      await updateDoc(fileRef, {
        content: content,
        lastModified: new Date()
      });
      
      // Also update the global files collection if needed
      const globalFileQuery = query(
        collection(db, 'files'),
        where('itemId', '==', file.id)
      );
      const querySnapshot = await getDocs(globalFileQuery);
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg p-6 w-full max-w-4xl h-3/4 flex flex-col">
        ...
        <textarea
          ...
          className="flex-1 w-full p-4 border border-gray-300 rounded resize-none focus:outline-none focus:ring-2 focus:ring-blue-500 text-gray-800"
...
const DashboardPage = () => {
  ...
  const deleteFolderContents = async (folderId) => {
    ...
    folderItemsSnapshot.forEach((doc) => {
      const item = { id: doc.id, ...doc.data() };
      deletePromises.push(deleteItem(item));
    });
    ...
    <div className="min-h-screen w-full bg-gray-900 text-white flex">
                ...
                displayItems.map((item) => (
                  ...
                  >
                    ...
                    <div className="text-5xl mb-4">
                      {item.type === "file" && !item.url ? "📝" : getFileIcon(item)}
                    </div>
                    <h3 className="text-lg font-semibold text-center mb-2">
                      {item.name}
                    </h3>
      ...
      {editingFile && (
        <FileEditor
          ...
          onSave={async () => {
            setEditingFile(null);
            // Refresh the items to show updated content
            const userItemsRef = collection(db, "users", user.uid, "items");
